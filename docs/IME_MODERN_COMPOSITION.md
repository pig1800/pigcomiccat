...
Priority rule: **A and B unblock the M2.5 gate** (item 6 in its modern form) and M5 proceeds on the IMM32 path. C is scheduled after M6 (editor + markers solid) — **unless** Phase A diagnostics show ATOK provides nothing via IMM32 even in-message, in which case C jumps ahead of M5 for the owner's daily-driver IME.
