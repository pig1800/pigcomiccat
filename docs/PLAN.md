...
- **Delivered 2026-08-25**: `CharacterMasterWindow` (ListBox of §6.3 rows: Original / Localized / Pronunciation / image cell with preview + Paste + Browse / gender / age / chapter / pronoun / comments; delete with confirm; inline duplicate-name rejection; unnamed rows are draft-only). Ctrl+V pastes the clipboard bitmap into the SELECTED row (Avalonia 12 API: `IClipboard.TryGetBitmapAsync`); images are downscaled to ≤256×256 preserving aspect ratio and stored as a PNG BLOB in `characters.db` (SQLite, `CharacterStore`). Tab cycles fields within a row (`KeyboardNavigation.TabNavigation="Cycle"`). Reachable from the project view ("Master characters") and from the character box's "Add to master?" offer (prefilled), plus Ctrl+Shift+M from the editor. DataGrid not used — no Avalonia.DataGrid package; a ListBox template keeps it to existing dependencies.
...
- **Delivered 2026-08-25**: name field (PartTextEditor, Enter commits; ↓/↑ move the highlight; Esc dismisses), suggestions over the master list (prefix-first then substring; prefix search over `CharacterStore`; missing/corrupt master degrades silently), chapter-name ToggleButtons (auto-highlight on the selected bubble's speaker; distinct names from `<characters>` + `@character`), `ApplyCharacter` → `SetCharacter` (adds to the chapter list) + dirty + header refresh; a brand-new name raises the inline "Add 『X』 to master list?" [Add/Not now] — Add opens the master editor prefilled. "✕ Clear" button always first (clears the speaker); right-click a chapter button → "Remove from chapter list" (inline ContextMenu per button). Character info block (read-only aligned form: Original/Localized/Pronounced/Gender/Age/Chapter/Pronoun/Comments, pure black) sits between the TM/TB box and the Kind selector.
...
- **Delivered 2026-08-25**: 6 kind ToggleButtons + an inline "Custom…" textbox (Speech/Thought/Narration/SFX/Sign/Note; matching raw kind highlights the button; arbitrary value allowed — D-59), notes PartTextEditor (multi-line, Enter=newline, write-through `SetNotes` + dirty), read-only LLM comment + Clear button (`ClearLlmComment`), Ctrl+Shift+K/C/N focus jumps (KeyBindings + EditorView routing). The pane's DataContext is now `FunctionPaneViewModel` (owns `Matches`, `Characters`); smoke updated to the composite. **D-60 focus rules**: keyboard-driven focus puts the caret at 0 with nothing selected; Left at pos 0 / Right at pos n are swallowed (never leave the textbox); Up/Down at the line boundary cross to the prev/next textbox.
